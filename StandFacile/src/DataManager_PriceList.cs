...
namespace StandFacile
{
    ...
    public partial class DataManager
    {
         ...
         ***************************************************************************/
        private bool bCaricaListino(bool bGetDataParam = false)
        {
            bool bChecksumTrovato = false;
            int s_pos; // posizione della semicolon
            int i, j, iPrzTmp, iDest;
            int iDiscType, iScontoFlag = 0, iScontoVal = 0, iCopertoVal = 0;
            ...
            try
            {
                ...
                if (bCheckIf_CassaSec_and_NDB())
                {
                    ...
                    if (File.Exists(sDir + NOME_FILE_LISTINO) && !bCheckService(Define._AUTO_SEQ_TEST))
                ...
                for (_ErrMsg.iRiga = 0; _ErrMsg.iRiga < sInputStrings.Count;)
                {
                    ...
                    else
                    {
                        if (String.IsNullOrEmpty(sInStr))   // riga vuota
                        {
                            ...
                            {
                                iRiga++;
                                uLocHashCode += uHash(_ErrMsg.iRiga); // Righe vuote calcola il checksum del numero per rivelare le inversioni
                                continue;
                            }
                            else
                                continue;
                    ...
                    if ((iDest >= 0) && (iDest < NUM_COPIES_GRPS))
                    {
                        ...
                        if (iDest != (int)DEST_TYPE.DEST_TIPO8)
                            uWebHashCode += uHash(sInStrCopy, iRiga);
                        else
                            sTmp = sInStrCopy;
                    }
                ...
                // if (_bDB_InitialConnectionOk && _bChecksumListinoCoerente && _bUSA_NDB())
                if (_bDB_InitialConnectionOk && _bUSA_NDB()) // meglio salvare anche con listino incoerente per evitare disallineamenti !
                {
                ...
                if (_bDB_InitialConnectionOk && _bChecksumListinoCoerente && dBaseTunnel_my.bGetWebServiceReq())
                {
        ...
        /// <summary>
        /// Non utilizzato ma serve solo come riferimento di confronto per StandFacile_V6
        /// </summary>
        private bool bCaricaListinoOld(bool bGetDataParam = false)
        {
            bool bChecksumTrovato = false;
            int s_pos; // posizione della semicolon
            int i, j, iPrzTmp, iDest;
        ...
        /// </summary>
        public static bool bSalvaListino()
        {
            bool bListinoIsGood;
            ...
            if (bCheckIf_CassaSec_and_NDB() || bCheckService(Define._AUTO_SEQ_TEST)) // cassa secondaria e DB, seqTest
                return false;
            ...
            else
            {
                ...
                // sconti
                int iDebug = _getSconto().iStatusSconto & 0x0000FF00;

                fPrz.WriteLine(";\n; sconti");
                iRowIndex += 2;

                sPrzRow = String.Format("#SC0 {0}; {1}; {2}", _getSconto().iScontoValPerc, (_getSconto().iStatusSconto & 0x0000FF00) >> 8, _getSconto().sScontoText[(int)DISC_TYPE.DISC_STD]);
                fPrz.WriteLine(sPrzRow);
                uLocHashCode += uHash(sPrzRow);

                sPrzRow = String.Format("#SC1 {0}; {1}", IntToEuro(_getSconto().iScontoValFisso), _getSconto().sScontoText[(int)DISC_TYPE.DISC_FIXED]);
                fPrz.WriteLine(sPrzRow);
                uLocHashCode += uHash(sPrzRow);

                sPrzRow = String.Format("#SC2 {0}", _getSconto().sScontoText[(int)DISC_TYPE.DISC_GRATIS]);
                fPrz.WriteLine(sPrzRow);
                uLocHashCode += uHash(sPrzRow);
                iRowIndex += 3;
                ...
                for (i = 0; i < _iLastArticoloIndexP1; i++)
                {
                    ...
                    if ((SF_Data.Articolo[i].iPrezzoUnitario > 0) || (SF_Data.Articolo[i].iGruppoStampa == (int)DEST_TYPE.DEST_COUNTER))
                    {
                        ...
                        if (SF_Data.Articolo[i].iGruppoStampa != (int)DEST_TYPE.DEST_TIPO8)
                            uWebHashCode += uHash(sPrzRow, i); // i per evitare inversioni di righe a parità di uWebHashCode
                        else
            ...
            if (dBaseTunnel_my.bGetWebServiceReq() && dBaseTunnel_my.rdbPing())
            {
                if ((SF_Data.iNumCassa == CASSA_PRINCIPALE) && (_sRemDBChecksum != _sWebListinoChecksum) && dBaseTunnel_my.bGetWebServiceReq())
                {
        ...
        public static void SalvaDati()
        {
            ...
            else
            {
                 ...
                 ***************************************************************/
                SF_Data.iNumOfLastReceipt = iGetNumOfOrders();
